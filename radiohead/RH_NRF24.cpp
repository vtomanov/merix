...
// bool RH_NRF24::waitPacketSent() altered with safty timeout (100msec) by Vasko Tomaniov 
// as found to have issues with some NRF24 chips behavior 
...
bool RH_NRF24::waitPacketSent()
    {
      ...
      uint8_t status;
      for (uint32_t c = 0;
           !((status = statusRead()) & (RH_NRF24_TX_DS | RH_NRF24_MAX_RT));
           c++)
      {
        if ((c & 255) == 0)
        {
